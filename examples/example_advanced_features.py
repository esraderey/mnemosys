...
from mneme import CompressionLevel, MnemeConfig, ZSpace
...
def demonstrate_safetensors():
    ...
    # Serialización a bytes con safetensors (sin archivos temporales)
    ...
    start_time = time.time()
    safetensors_data = save({"tensor": tensor})
    serialize_time = time.time() - start_time

    # Deserializar con safetensors
    start_time = time.time()
    loaded_data = load(safetensors_data)
...
def demonstrate_lazy_decompression(mneme):
    ...
    if hasattr(desc, 'lazy_tensor') and desc.lazy_tensor:
        ...
        error = torch.norm(large_tensor - decompressed.cpu()) / torch.norm(large_tensor)
        print(f"✅ Error de reconstrucción (SVD con pérdida): {error:.6f}")

        # Limpiar memoria decompressed
        desc.lazy_tensor.clear_decompressed()
        print("🧹 Memoria decompressed liberada")
    else:
        print("(este descriptor no expone lazy tensor; nada que demostrar)")
